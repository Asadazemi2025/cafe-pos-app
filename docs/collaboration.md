...
**ターミナル** … 「アプリケーション → ユーティリティ → ターミナル」、または Command + スペース で「ターミナル」と検索。

**Git** … 最初から入っているように見えて、初回は追加のインストールが必要。`git --version` を打つと「開発者用ツールをインストールしますか」と聞かれるので「インストール」を押す(数分)。

**`.env` の作り方** … MacのテキストエディットはRTF形式になったり `.txt` が付いたりして失敗しやすいので、ターミナルで作るのが確実。
...
黒い画面に切り替わるので、もらった中身を Command + V で貼り付け、

1. `Control + O` を押す → Enter（保存）
2. `Control + X` を押す（終了）
...
貼り付けた内容が表示されればOK。なお Finder では `.` で始まるファイルは隠れている。`Command + Shift + .`（ピリオド）で表示/非表示を切り替えられる。
